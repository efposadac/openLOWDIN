	...
	HDF5VER="${HDF5MAJOR}.${HDF5MINOR}"
        if [ ! -d ${builddir}/hdf5-${HDF5VER} ]; then
            if [ ! -f ${srcdir}/hdf5-${HDF5VER}.tar.gz ]; then
                cd ${srcdir}
		wget -O hdf5-${HDF5VER}.tar.gz https://support.hdfgroup.org/ftp/HDF5/prev-releases/hdf5-${HDF5MAJOR}/hdf5-${HDF5VER}/src/hdf5-${HDF5VER}.tar.gz
...
# BLAS
if(( ! ${system_blas} )); then
    LAPACKSER="-L${topdir}/openblas/lib/ -lopenblas -lgfortran -lquadmath"
    LAPACKOMP="-L${topdir}/openblas/lib/ -lopenblaso -lgfortran -lquadmath"
fi
...
# make erkale_loc erkale_fchkpt -j ${nprocs} VERBOSE=1
# make install
# cd ..
