...
contains
  ...
  subroutine CCSD_init(speciesId)
      ...
      !         !under construction        
              ...
              ! write(*,*) CCSD_instance%Tdsame(a-nop,b-nop,i,j), "Tdsame"
          ...
          do m=1, nop
            ...
            do f=nop+1, noc
              ...
              do n=1, nop
                ...
                ! write(*,*) a-nop,f-nop,m,n,spints(speciesId)%valuesp(m,n,e,f)*(-0.5*CCSD_instance%ttau(a-nop,f-nop,m,n))
              end do
            end do
          end do
        end do
          ...
          do e=nop+1, noc
            ...
            do n=1, nop
              ...
              do f=nop+1, noc
                ...
                write(*,*) a,e,e-nop,f-nop,0.5*CCSD_instance%ttau(e-nop,f-nop,i,n)*spints(speciesId)%valuesp(m,n,e,f)
              end do
            end do
          end do
        end do
  ...
  subroutine CCSD_loop(speciesId)
      ...
      if (convergence /= 1.0D-8) convergence = 1.0D-8
  ...
  subroutine CCSD_run()
      ...
      print*, "INFORMATION IN CCSD_constructor() Total CCSD energy: ", CoupledCluster_instance%CCSD_ones_Energy &
