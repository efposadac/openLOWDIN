...
contains
  ...
  subroutine RysQuadrature_computeIntraSpecies(specieID, job, starting, ending, process)
    ...
    call Matrix_constructor(coefficients, int(totalNumberOfContractions,8), int(totalNumberOfContractions,8))
    ...
    coefficients= Matrix_getFromFile(unit=wfnUnit, rows= int(totalNumberOfContractions,4), &
...
!     !! allocating space for integrals just one time (do not put it inside of loop!!!)
!     arraySize = ((maxAngularMoment + 1)*(maxAngularMoment + 2))/2
...
!                    !! Calculate the size of integral (ab|rs)
!                    arraySize = contractions(a)%numCartesianOrbital * &
!                         contractions(b)%numCartesianOrbital * &
!                         contractions(r)%numCartesianOrbital * &
!                         contractions(s)%numCartesianOrbital
...
!                                m = 0
!                                do nca = 1, contractions(a)%numCartesianOrbital
!                                   do ncb = 1, contractions(b)%numCartesianOrbital
!                                      do ncr = 1, contractions(r)%numCartesianOrbital
!                                         do ncs = 1, contractions(s)%numCartesianOrbital
!                                            m = m + 1
                                           
!                                            integralsPtr(m) = integralsPtr(m) &
!                                                 * contractions(a)%primNormalization(i,nca) &
!                                                 * contractions(b)%primNormalization(j,ncb) &
!                                                 * contractions(r)%primNormalization(k,ncr) &
!                                                 * contractions(s)%primNormalization(l,ncs)
...
!                                integralsPtr(1:arraySize) = integralsPtr(1:arraySize) * &
!                                     contractions(a)%contractionCoefficients(i) * &
!                                     contractions(b)%contractionCoefficients(j) * &
!                                     contractions(r)%contractionCoefficients(k) * &
!                                     contractions(s)%contractionCoefficients(l) 
                               
!                                     integralsValue(1:arraySize) = integralsValue(1:arraySize) + integralsPtr(1:arraySize)
...
!                                !!normalize by shell
!                                integralsValue(m) = integralsValue(m) &
!                                     * contractions(a)%contNormalization(nca) &
!                                     * contractions(b)%contNormalization(ncb) &
!                                     * contractions(r)%contNormalization(ncr) &
!                                     * contractions(s)%contNormalization(ncs)                            
                               
                               
!                                !!write to disk
!                                pa=labelsOfContractions(a)+nca-1
!                                pb=labelsOfContractions(b)+ncb-1
!                                pr=labelsOfContractions(r)+ncr-1
!                                ps=labelsOfContractions(s)+ncs-1
