...
contains
  ...
  subroutine MolecularSystem_showHSDFormat()
    ...
    do i = 1, MolecularSystem_instance%numberOfQuantumSpecies
       ...
       numberOfAtoms=j-1
       !       print *, numberOfAtoms
       open(unit=8, file="LOWDIN.xyz")
       write (8,*) numberOfAtoms
       write (8,*) " "
    end do
    ...
    do i = 1, MolecularSystem_instance%numberOfPointCharges
       ...
       ! write (8,*) numberOfAtoms
       ! write (8,*) " "
       write (8,"(A10,3F20.10)") trim(MolecularSystem_instance%pointCharges(i)%nickname), origin(1), origin(2), origin(3)
    end do


    call system("xyz2gen LOWDIN.xyz")
    print *, ""
    print *, "Geometry converted to gen file."
    print *," "
    
    open(unit=9, file="dftb_in.hsd")
    write (9,*) 'Geometry = GenFormat {'
    write (9,*) '  <<< "LOWDIN.gen"'
    write (9,*) '}'
    write (9,*) ' '
    write (9,*) 'Driver = {}'
    write (9,*) ''
    write (9,*) 'Hamiltonian = DFTB {'
    write (9,*) 'SCC = Yes'
    write (9,*) 'SCCTolerance = 1.00E-6'
    write (9,*) 'MaxSCCIterations = 1000'
    write (9,*) 'MaxAngularMomentum {'
    write (9,*) 'H = "s"'
    write (9,*) 'O = "p"'
    write (9,*) '}'
    write (9,*) 'SlaterKosterFiles = Type2FileNames {'
    write (9,*) 'Prefix = "/opt/dftbplus-18.2/external/slakos/origin/mio-1-1/"'
    write (9,*) 'Separator = "-"'
    write (9,*) 'Suffix  = ".skf"'
    write (9,*) '}'
    write (9,*) 'Dispersion = {}'
    write (9,*) 'ThirdOrder = Yes'
    write (9,*) 'HubbardDerivs {'
    write (9,*) 'O = -0.14'
    write (9,*) 'H = -0.07'
!    write (9,*) 'C = -0.04'
    write (9,*) '}'
    write (9,*) 'HCorrection = Damping {'
    write (9,*) 'Exponent = 4.05'
    write (9,*) '}'
    write (9,*) '}'
    write (9,*) ''
    write (9,*) 'Options = {}'
    ! write (9,*) 'WriteAutotestTag = Yes'
