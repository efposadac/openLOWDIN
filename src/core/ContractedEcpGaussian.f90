...
!! </table>
!!
!! @author Ismael Ortiz-Verano
!!
!! <b> Fecha de creacion : </b> 2017-03-23
!!
...
module ContractedEcpGaussian_
  ...
  type :: contractedEcpGaussian
     integer :: id
     integer :: length
     integer :: angularMoment
     integer :: numCartesianOrbital ! (l+1)(l+2)/2
     integer :: maxAngularMoment
     integer :: owner
     real(8) :: origin(3)
          real(8) , allocatable :: contNormalization(:)
     integer , allocatable :: nkParameters(:)
     real(8) , allocatable :: zetakParameters(:)
     real(8) , allocatable :: dkParameters(:)
          real(8) , allocatable :: primNormalization(:,:)
  end type contractedEcpGaussian


  public :: &
       ContractedEcpGaussian_saveToFile, &
       ContractedEcpGaussian_loadFromFile, &
   ContractedEcpGaussian_showInCompactForm, &
  ! ContractedCoreGaussian_showInSimpleForm,  &
  ! ContractedCoreGaussian_normalizePrimitive, &
  ! ContractedCoreGaussian_normalizeContraction, &
  ! ContractedCoreGaussian_overlapIntegral, &
   ContractedEcpGaussian_getShellCode!, &
  ! ContractedCoreGaussian_getAllAngularMomentIndex
...
contains
  ...
  subroutine ContractedEcpGaussian_saveToFile(this, unit)
    implicit none

    type(ContractedEcpGaussian) :: this
    integer :: unit
...
#ifdef intel
    write(unit,'(<size(this%nkParameters)>F)') this%nkParameters
    write(unit,'(<size(this%zetakParameters)>F)') this%zetakParameters
    write(unit,'(<size(this%dkParameters)>F)') this%dkParameters
    !    write(unit,'(<size(this%primNormalization)>F)') this%primNormalization
#else
    write(unit,*) this%nkParameters
    write(unit,*) this%zetakParameters
    write(unit,*) this%dkParameters
    !    write(unit,*) this%primNormalization
#endif

  end subroutine ContractedEcpGaussian_saveToFile

  !>
  !! @brief Loads the ContractedGaussian structure from file.
  !! @param this contracted gaussian
  !! @author E. F. Posada, 2013
  subroutine ContractedEcpGaussian_loadFromFile(this, unit)
    implicit none
    ...
    read(unit,*) this%maxAngularMoment
    read(unit,*) this%owner
    read(unit,*) this%origin

    allocate(this%nkParameters(this%length))
    allocate(this%zetakParameters(this%length))
    allocate(this%dkParameters(this%length))

    read(unit,*) this%nkParameters
    read(unit,*) this%zetakParameters
    read(unit,*) this%dkParameters

    print*, "nkParameters: ",this%nkParameters
  ...
  end subroutine ContractedEcpGaussian_loadFromFile
  ...
  subroutine ContractedEcpGaussian_showInCompactForm( this )
    implicit none

    type(ContractedEcpGaussian) , intent(in) :: this
  ...
  end subroutine ContractedEcpGaussian_showInCompactForm
  ...
  !! @version 1.0
  function ContractedEcpGaussian_getShellCode( this ) result( output )
    implicit none
    type(ContractedEcpGaussian) , intent(in) :: this
...
!           print*,this%angularMoment,"angularMoment"
  ...
  end function ContractedEcpGaussian_getShellCode
  ...
  subroutine ContractedEcpGaussian_exception( typeMessage, description, debugDescription)
    implicit none
    integer :: typeMessage
  ...
  end subroutine ContractedEcpGaussian_exception

end module ContractedEcpGaussian_
