...
!    wfnFile = trim(CONTROL_instance%INPUT_FILE)//"lowdin.vec"
    ...
    if ( .not.CONTROL_instance%ELECTRONIC_WAVEFUNCTION_ANALYSIS ) then
       ...
       call SingleSCF_convergenceMethod(wfObject)
       ...
       if ( CONTROL_instance%ACTIVATE_LEVEL_SHIFTING .eqv. .true. ) then
          ...
          if ( MolecularSystem_instance%species(wfObject%species)%isElectron ) then
          ...
          fockMatrixTransformed%values = &
               matmul( matmul( transpose(wfObject%waveFunctionCoefficients%values ) , &
               fockMatrixTransformed%values), wfObject%waveFunctionCoefficients%values )

          totales = numberOfContractions !Matrix_getNumberOfRows(fockMatrixTransformed)
          ocupados = MolecularSystem_getOcupationNumber(wfObject%species )
          ...
          fockMatrixTransformed%values = &
               matmul( matmul(wfObject%waveFunctionCoefficients%values, &
               fockMatrixTransformed%values), transpose(wfObject%waveFunctionCoefficients%values ))
       ...
       if ( CONTROL_instance%ACTIVATE_LEVEL_SHIFTING .eqv. .true. ) then
          do i= ocupados + 1, totales
             wfObject%molecularOrbitalsEnergy%values(i) = wfObject%molecularOrbitalsEnergy%values(i) -levelShiftingFactor
          end do
       end if
      !!
       !! Interation ends
       ...
       if ( CONTROL_instance%FREEZE_NON_ELECTRONIC_ORBITALS) then
          if ( CONTROL_instance%READ_COEFFICIENTS ) then
             if ( .not. MolecularSystem_instance%species(wfObject%species)%isElectron ) then

                inquire(FILE = trim(wfObject%name)//".vec", EXIST = existFile )

                if ( existFile) then

                   wfObject%waveFunctionCoefficients = Matrix_getFromFile(int(numberOfContractions,4), int(numberOfContractions,4), &
                        file=trim(wfObject%name)//".vec", binary = .false.)
  ...
  subroutine SingleSCF_convergenceMethod(wfObject)
    implicit none
    type(WaveFunction) :: wfObject

    !!**************************************************************************************************
    !! Emplea metodo de convergecia SCF
    if (  wfObject%numberOfIterations  > 1  )then
       if ( Convergence_isInstanced(wfObject%convergenceMethod ) ) then
       ...
       call Convergence_setInitialDensityMatrix(wfObject%convergenceMethod, &
            wfObject%densityMatrix )
       call Convergence_setInitialFockMatrix(wfObject%convergenceMethod, &
            wfObject%fockMatrix )
    end if
    !!
    !!**************************************************************************************************
