...
contains
  ...
  subroutine MultiSCF_obtainFinalEnergy(this, wfObjects, libint2Objects, method)
    ...
    if (CONTROL_instance%LOCALIZE_ORBITALS) then
      ...
      write (*, *) ""
      do speciesID = 1, numberOfSpecies
        if (MolecularSystem_getMass(speciesID, this%molSys) .lt. 10.0 .and. MolecularSystem_getOcupationNumber(speciesID, this%molSys) .gt. 1) then 
          !We assume that heavy particle orbitals are naturally localized
