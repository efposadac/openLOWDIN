  ...
  subroutine CONTROL_show()
    ...
    if (CONTROL_instance%NONORTHOGONAL_CONFIGURATION_INTERACTION) then
      ...
      if (sum(CONTROL_instance%TRANSLATION_SCAN_GRID) .gt. 0) then
           write (*,"(T10,A,I6,A)") "THE BASIS FUNCTIONS AT EACH TRANSLATION CENTER WILL BE DISPLACED FORMING ", &
                                    CONTROL_instance%TRANSLATION_SCAN_GRID(1)*CONTROL_instance%TRANSLATION_SCAN_GRID(2)*CONTROL_instance%TRANSLATION_SCAN_GRID(3) ," BODY-CENTERED-CUBIC CELLS"
      ...
      if (CONTROL_instance%ROTATION_AROUND_Z_STEP .gt. 0) then
        ...
        !    write (*,"(T10,A,F6.3,A10)") "WITH A RADIAL SEPARATION  OF", CONTROL_instance%NESTED_GRIDS_DISPLACEMENT,  " BOHRS"
        ! else
        write (*, "(T10,A,F8.2,A,F7.4,A)") "THE MOLECULAR SYSTEM WILL BE ROTATED AROUND THE Z AXIS IN STEPS OF", CONTROL_instance%ROTATION_AROUND_Z_STEP, &
        " DEGREES UP TO ", CONTROL_instance%ROTATION_AROUND_Z_MAX_ANGLE, " DEGREES"
        ! end if
      end if
