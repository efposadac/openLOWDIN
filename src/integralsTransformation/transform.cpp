 ...
 *     No portion of this file may be distributed, modified, or used without express
...
static void Similarity_Transform(unsigned n,double Op[],double C[],double Work[])
{
  char * not_t = "N";
  char * yes_t = "T";
  int n_copy=n;
  ...
  unsigned i,j,k,ijr,ik,kj,ki;
  double zero = 0;

  memset(Work,0.0,n*n*sizeof(double));

  dgemm_(yes_t,not_t,&n_copy,&n_copy,&n_copy,&one,C,&n_copy,Op,&n_copy,&zero,Work,&n_copy);

  
  dgemm_(not_t,not_t,&n_copy,&n_copy,&n_copy,&one,Work,&n_copy,C,&n_copy,&zero,Op,&n_copy);
